...
## Named Profile Migration — Phased Implementation

**Goal:** Replace the 20 hardcoded synthetic test users with a CSV-driven data layer that also includes
63 real SiriusXM employee profiles (named profiles) with actual recommendation data sourced from
Databricks. All user–entity relationships are stored as entity IDs (GUIDs) and resolved at runtime
via reference CSVs.

### New `data/users.csv` Schema (Phase 1+)

| Column | Type | Notes |
|--------|------|-------|
| `email_address` | string | `firstname.lastname@test.com` for test users; `@siriusxm.com` for named profiles |
| `profile_id` | UUID string | Deterministic hash-UUID for test users; real profile_id for named profiles |
| `first_name` | string | |
| `last_name` | string | |
| `phone` | string | E.164 format; empty string if unknown. Named profiles: `+15550010100` incrementing |
| `subscription_tier` | string | `trial\|select\|premier\|all-access\|expired`. Named profiles: always `premier` |
| `trial_end_date` | string | ISO date string; empty for non-trial. Named profiles: always empty |
| `top_recommendation` | JSON array string | `["uuid","uuid",...]` — ranked entity IDs (all types: channels, shows, podcasts, etc.) |
| `top_artists` | JSON array string | `["uuid","uuid",...]` — talent/artist-station entity IDs |
| `top_teams` | JSON array string | `["uuid","uuid",...]` — team entity IDs; empty for test users |

Array fields are stored as quoted JSON arrays in the CSV cell: `"[""uuid1"",""uuid2""]"`

### Source CSVs and their roles

| CSV | Used for |
|-----|---------|
| `data/named_profile_users.csv` | Named profile identity (profile_id, first/last name, email) |
| `data/named_profile_team_hmf_for_you_recommendations.csv` | Named profile `top_recommendation` |
| `data/named_profile_team_hmf_arstists_recommendations.csv` | Named profile `top_artists` (note: camelCase `profileId` column) |
| `data/named_profile_team_recommendations.csv` | Named profile `top_teams` |
| `data/sxm_channel_reference.csv` | Fuzzy-match channel names → entity IDs (for existing test users) |
| `data/sxm_talent_content_relationship_ref.csv` | Fuzzy-match artist names → talent entity IDs (for existing test users) |

### CSV generation script: `generate-users-csv.mjs`

Reads all source CSVs above and writes two files. Run once whenever source data changes:
```bash
node generate-users-csv.mjs
```
Outputs:
- `data/users.csv` — canonical data (83 users: 20 test + 63 named)
- `data/users-data.ts` — TypeScript version; imported by `synthetic-data.ts` at build time

**Architecture note — why `users-data.ts` instead of runtime `readFileSync`:**
The Sierra bundler (esbuild) targets a browser-like runtime. Node.js built-ins (`fs`, `path`) are not
resolvable in the bundle. Data must be embedded at build time, identical to the `sxm-catalog.ts` pattern.
`generate-users-csv.mjs` therefore emits TypeScript code (not just CSV) and `synthetic-data.ts` imports it.

**Existing test users — data source:**
Rather than fuzzy-matching from the old `users.csv` (which had approximate channel/artist names),
`generate-users-csv.mjs` hardcodes the 20 existing users' exact entity IDs from `synthetic-data.ts`.
This avoids any name-to-entity-ID mismatch. The old `data/users.csv` schema is now superseded.

**UUID assignment for existing test users:**
Deterministic: `crypto.createHash('md5').update(userId).digest('hex')` formatted as UUID.
Ensures the same profile_id is produced on every script run (USR001 → `825a3824-78c1-d865-8cab-d4c80bbcfffd`).

**Phone number assignment for named profiles:**
Incremental from `+15550010100` in the order they appear in `named_profile_users.csv`
(sorted by last_name, first_name as generated by the SQL query).

### `data/synthetic-data.ts` — generated data import (Phase 1 ✅)

Imports `GENERATED_USER_PROFILES` from `data/users-data.ts` (the TypeScript-embedded version).

**CSV column → `UserProfileRecord` field mapping (Phase 1 transition names):**

| CSV column | TS field | Notes |
|------------|----------|-------|
| `profile_id` | `userId` | Field rename happens in Phase 3 |
| `email_address` | `email` | |
...
| `top_recommendation` | `topChannels` | Transition name; renamed to `topRecommendation` in Phase 3 |
| `top_artists` | `topArtists` | Phase 1: names for test users, entity IDs for named profiles. Full entity IDs in Phase 2 |
| `top_teams` | `topTeams` | New additive field |
...
| Field | Resolved via |
|-------|-------------|
| `topChannels` / `topRecommendation` | `getChannelByKey(entityId)` → `sxm_channel_reference.csv` data |
| `topArtists` (Phase 2+) | `getTalentById(entityId)` → `sxm_talent_reference.csv` data |
| `topTeams` | Team name already embedded in `named_profile_team_recommendations.csv`; future: lookup table |

### Test categories
- `phase-named-profiles-p1` — CSV loading, profile resolution, topTeams presence
- `phase-named-profiles-p2` — artist entity ID → affinity tag and talent card
- `phase-named-profiles-p3` — topRecommendation field rename regression

### Done when (all three phases)
- `pnpm sierra build` passes after each phase
- `pnpm sierra test --categories phase10` still passes (no regression)
- Named profile (e.g. `rory.belfi@siriusxm.com`) resolves correctly in agent
- Named profile channel recommendations show real channel names
- Named profile with teams → sports team surfaced in conversation
