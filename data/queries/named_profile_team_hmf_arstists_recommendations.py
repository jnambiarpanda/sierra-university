# Generates: data/named_profile_team_hmf_arstists_recommendations.csv
# Fetches the latest coffee_artists4you recommendations per named profile,
# decompresses the gzipped base64 JSON payload, resolves each artist entity ID
# to a name via refined_prod.content_ingestion (talents / artist_stations / teams),
...
df = spark.sql(f"""
    ...
    FROM tenant_prod.mli_recommendations.for_you_online_rankers_dbx_coffee_artists4you
...
# ── Decompress, parse JSON array, explode, extract id ─────────────────────────
artist_schema = ArrayType(StructType([
...
df_exploded = (
    df
    .withColumn('artists_json', decompress_udf(F.col('json')))
    .withColumn('artists_array', F.from_json(F.col('artists_json'), artist_schema))
    .select('profileId', F.explode('artists_array').alias('artist'))
    .select('profileId', F.col('artist.id').alias('entity_id'))
)

df_exploded.createOrReplaceTempView('exploded_artists')

# ── Join 3 content tables, resolve name/type, re-aggregate as JSON array ──────
result = spark.sql("""
    WITH enriched AS (
        SELECT
            e.profileId,
            e.entity_id,
            COALESCE(ta.name, ast.name, tm.name) AS entity_name,
            CASE
