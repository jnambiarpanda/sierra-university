-- Generates: data/named_profile_team_recommendations.csv
-- Fetches the latest favorite teams per named profile, enriches each team ID
-- with name from refined_prod.content_ingestion.teams, and outputs a JSON array.
...
ranked AS (
  SELECT
    ft.*,
    ROW_NUMBER() OVER (PARTITION BY ft.profile_id ORDER BY ft.ingestion_date DESC) AS rn
  FROM
    tenant_prod.mli_recommendations.for_you_online_rankers_dbx_favorite_teams ft
  WHERE
    ft.profile_id IN (
...
exploded AS (
  ...
  FROM
    latest l
    LATERAL VIEW explode(l.teams) te AS team_id
),
enriched AS (
  SELECT
    e.profile_id,
    NAMED_STRUCT('entity_id', e.team_id, 'entity_type', 'team', 'name', t.name) AS team_obj
  FROM
    exploded e
    LEFT JOIN refined_prod.content_ingestion.teams t
      ON e.team_id = t.id
)
SELECT
  profile_id,
  TO_JSON(COLLECT_LIST(team_obj)) AS teams_entity_id_array
FROM
  enriched
GROUP BY
  profile_id
