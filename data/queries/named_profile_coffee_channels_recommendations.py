# Generates: data/named_profile_coffee_channels_recommendations.csv
# Fetches the latest coffee_channels4you recommendations per named profile,
# decompresses the gzipped base64 JSON payload, splits semicolon-concatenated
# entity IDs, joins all 8 content type tables to resolve entity_type and name,
# and outputs a JSON array per profile.
...
PROFILE_IDS = [
    ...
    '452b8693-cad8-49ab-9532-b691eb0b2477',
]

# ── UDF: base64 decode → gzip decompress → return raw JSON string ─────────────
def decompress_json(b64_str):
    if b64_str is None:
        return None
    try:
        decompressed = gzip.decompress(base64.b64decode(b64_str))
        return decompressed.decode('utf-8')
    except Exception:
        return None

decompress_udf = F.udf(decompress_json, StringType())

# ── Pull latest snapshot for matching profiles ────────────────────────────────
profile_ids_str = ", ".join(f"'{p}'" for p in PROFILE_IDS)

df = spark.sql(f"""
    SELECT profileId, json
    FROM tenant_prod.mli_recommendations.for_you_online_rankers_dbx_coffee_channels4you
    WHERE ingestion_date = (
        SELECT MAX(ingestion_date)
        FROM tenant_prod.mli_recommendations.for_you_online_rankers_dbx_coffee_channels4you
    )
    AND profileId IN ({profile_ids_str})
""")

# ── Decompress, parse JSON array, explode, extract id, split on semicolons ────
channel_schema = ArrayType(StructType([
    StructField("id", StringType(), True),
    StructField("tp", StringType(), True),
    StructField("sc", DoubleType(), True)
]))

df_exploded = (
    df
    .withColumn('channels_json', decompress_udf(F.col('json')))
    .withColumn('channels_array', F.from_json(F.col('channels_json'), channel_schema))
    .select('profileId', F.explode('channels_array').alias('channel'))
    .select('profileId', F.col('channel.id').alias('raw_entity_id'))
    # Split semicolon-concatenated IDs into individual GUIDs
    .select('profileId', F.explode(F.split(F.col('raw_entity_id'), ';')).alias('entity_id'))
    .filter(F.trim(F.col('entity_id')) != '')
    .withColumn('entity_id', F.trim(F.col('entity_id')))
)

df_exploded.createOrReplaceTempView('exploded_channels')

# ── Join 8 content tables, resolve name/type, re-aggregate as JSON array ──────
result = spark.sql("""
    WITH enriched AS (
        SELECT
            e.profileId,
